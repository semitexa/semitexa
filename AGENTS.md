...
| One process in depth — spans, queries with positions, payload snapshot | `ai:observe show --id=<p-…>` *(inlines the full trace when the request carried `?__trace=1`)* |
...
| `ai:observe` | The Observatory: live process journal, deep traces, sandbox replay. Humans get the same data at `/__observatory`; a request with `?__trace=1` records a full waterfall at `/__trace` | **First stop when debugging runtime behaviour** — observe before hypothesizing. Dev-only by construction |
